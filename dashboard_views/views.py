from rest_framework import decorators, response, views, permissions, authentication
...
class ShopReportView(views.APIView):
      ...
      def get(self, request):
            ...
            if hasattr(user, 'shop'):
                  report = AnalysisReportSerializer.get_shop_report(user.shop)
            elif hasattr(user, 'branch'):
                  report = AnalysisReportSerializer.get_branch_report(user.branch)
            else:
                  return response.Response({'error': 'User does not belong to a shop or branch'}, status=400)
      
            return response.Response(report.data, status=200)
